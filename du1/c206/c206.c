...
void DLL_InsertAfter( DLList *list, int data ) {
    ...
    if (list->activeElement != NULL){
        ...
        if (tmp != NULL){
            tmp->data = data;
           if (list->activeElement == list->lastElement){ // kontrola zda nevkladame prvek za posledni
               list->lastElement->nextElement = tmp;
               tmp->previousElement = list->lastElement;
               tmp->nextElement = NULL;
               list->lastElement = tmp;
           } else { //pridani polozky mezi dva prvky kdy se nemeni lastElement
...
void DLL_InsertBefore( DLList *list, int data ) {
    ...
    if (list->activeElement != NULL){
        ...
        if (tmp != NULL){
            tmp->data = data;
            if (list->activeElement == list->firstElement){ // kontrola zda nevkladame prvek pred prvni
                list->lastElement->previousElement = tmp;
                tmp->nextElement = list->firstElement;
                list->firstElement = tmp;
            } else { //pridani polozky mezi dva prvky kdy se nemeni firstElement
